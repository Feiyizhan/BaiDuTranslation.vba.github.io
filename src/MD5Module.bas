...
Private Function ConvertToWordArray(sMessage)
    ...
    lMessageLength = LenB(sMessage)  '如果字符串编码是Unicode 那么用Len(sMessage)
    ...
    Do Until lByteCount >= lMessageLength
        ...
        lWordArray(lWordCount) = lWordArray(lWordCount) Or LShift(AscB(MidB(sMessage, lByteCount + 1, 1)), lBytePosition)  '如果字符串编码是Unicode 那么用Asc和Mid
...
Public Function MD5(sMessage, stype) As String
    m_lOnBits(0) = CLng(1)
    m_lOnBits(1) = CLng(3)
    m_lOnBits(2) = CLng(7)
    m_lOnBits(3) = CLng(15)
    ...
    Dim data() As Byte
    data = sMessage
    sMessage = BytesToBytesNoBom(data, "unicode", "utf-8") '将Unicode 编码的字符串转换为UTF-8
    data = sMessage
...
Public Function MD5_32(str As String) As String
    MD5_32 = MD5(str, 32)
End Function

Public Sub test()

    Debug.Print (MD5_32("20160126000009412苹果31005LkJ7MyN4Aml_HYjbnBzk"))
